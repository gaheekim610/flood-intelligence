...
fs.writeFileSync(csvPath, 'station_name,minor,moderate,major\n');
...
for (const file of htmlFiles) {
    ...
    while ((match = areaRegex.exec(content)) !== null) {
        ...
        const stationName = args[0].replace(/"/g, '').replace(/&quot;/g, '');
        // The last 4 variables are the flood levels (may be empty or ",")
        let minor = args[args.length - 4] ? args[args.length - 4].trim() : '';
        let moderate = args[args.length - 3] ? args[args.length - 3].trim() : '';
        let major = args[args.length - 2] ? args[args.length - 2].trim() : '';
        ...
        if (minor && minor !== ',' && minor.trim() !== '' && /\d/.test(minor)) {
            const row = `${stationName},${minor},${moderate},${major}\n`;
